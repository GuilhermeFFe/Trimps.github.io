...
function updateNatureInfoSpans(){
	for (var item in game.empowerments){
		...
		document.getElementById('uber' + item + "Name").innerHTML = ((Fluffy.isRewardActive('FluffyE14') || game.global.fluffyPrestige >= 15) ? "<span class='green'>Activated by Fluffy</span>" : (emp.getLevel() < 50) ? "Locked" :  (game.global.uberNature != "") ? "<span class='red'>" + game.global.uberNature + " active</span>" : "Enlightened " + item);
		var unlockZone = 235;
...
function startFight() {
    ...
    if (game.global.soldierHealth <= 0) {
		...
		if (getPerkLevel("Anticipation")){
			...
			if(game.global.challengeActive == "Finale"){game.global.antiStacks = 0;
			}else if (game.talents.patience.purchased){
...
function giveSpireReward(level){
	...
	switch(level){
		...
		case(100):
			...
			if(spireWorld == 8){
				text += "Druopitinity is fallen. You found the real key of your ship! Also, enemy stats are reduced, but your equipment stats are reduced by same amount.";
			...
			}else if(game.global.challengeActive == "Spired")text += "You have found a Spire Core Heirloom that worth "+Math.floor((game.global.world**3)*Math.pow(10,(game.global.world-300)/100)*(game.global.world%100==0?(Fluffy.isRewardActive("FluffyE19")?30:10):1)*(u2Mutations.tree.Nullifium.purchased?1.1:1)*(Fluffy.isRewardActive("FluffyE19")?3:1))+" Spirestones!";
			...
			if (spireWorld == 6){
				...
				text += "<br/><span class='fullDarkEssence'>At the top of this Spire, you found a gigantic, pitch-black cache with " + prettify(1e40) + " Dark Essence inside!</span><br/>";
				//	}
				//}
			}
	...
	switch(level){
		...
		case 100:
			...
			if (game.global.spiresCompleted < 1){
				...
				else text += "<br/><br/><b>You have found a Spire Core Heirloom that worth "+(u2Mutations.tree.Nullifium.purchased?8800000:8000000)+" Spirestones!</b>";
			}
			}else{
				createHeirloom(200, false, true);
				if(game.global.world>=230)text += "You have found a Spire Core Heirloom that worth "+Math.floor((game.global.world**3)*Math.pow(10,(game.global.world-300)/100)*(u2Mutations.tree.Nullifium.purchased?1.1:1))+" Spirestones!";
				else text += "You have found a Spire Core Heirloom that worth "+((u2Mutations.tree.Nullifium.purchased?22:20)*game.global.world*game.global.world)+" Spirestones!";
			}
...
var Fluffy = {
	...
	tooltip: function (big){
		...
		if (showCruffys && game.global.challengeActive != "Nurture"){
			...
			if (savedLevel > fluffyInfo[0]) topText += "<span class='red'>- " + name + "'s level and damage bonus are currently reduced. " + name + " will return to level " + savedLevel + " when points are placed back in Capable.</span>";
				...
				if(game.global.fighting||game.global.world!=1){
					topText += "<span class='red'>- " + name + " cannot gain any Experience in Finale Challenge, because he is fighting for you in this Challenge!</span>";
				}else{
					if(game.jobs.Farmer.owned)topText += "- " + name + " is gathering food for you in the Finale Challenge.<br/>";
					if(game.jobs.Lumberjack.owned)topText += "- " + name + " is gathering wood for you in the Finale Challenge.<br/>";
					if(game.jobs.Miner.owned)topText += "- " + name + " is gathering metal for you in the Finale Challenge.<br/>";
					if(game.jobs.Scientist.owned)topText += "- " + name + " is gathering science for you in the Finale Challenge.<br/>";
					if(game.jobs.Explorer.owned)topText += "- " + name + " is gathering fragments for you in the Finale Challenge.<br/>";
			...
			if(game.global.fighting||game.global.world!=1){
				topText += "- " + name + " is fighting for you in the Finale Challenge!";
			}else{
				if(game.jobs.Farmer.owned)topText += "- " + name + " is gathering food for you in the Finale Challenge.<br/>";
				if(game.jobs.Lumberjack.owned)topText += "- " + name + " is gathering wood for you in the Finale Challenge.<br/>";
				if(game.jobs.Miner.owned)topText += "- " + name + " is gathering metal for you in the Finale Challenge.<br/>";
		...
		if (Fluffy.canGainExp() && game.global.world >= minZoneForExp && (!showCruffys || fluffyInfo[0] < 19)) {
			...
			if ((getUberEmpowerment() == "Ice" || getUberEmpowerment() == "ALL")) fluffFormula += ' * <span class="fluffFormIce" onmouseover="Fluffy.expBreakdown(\'ice\')" onmouseout="Fluffy.expBreakdown(\'clear\')">Ice</span>';
