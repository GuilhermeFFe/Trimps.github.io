...
function tooltip(what, isItIn, event, textString, attachFunction, numCheck, renameBtn, noHide, hideCancel, ignoreShift) { //Now 20% less menacing. Work in progress.
	...
	if (what == "The Spire"){	
		if (game.global.challengeActive == "Spired"){
			tooltipText = "";
			if(game.global.world <= 1)tooltipText += "<span class='planetBreakMessage'>Welcome to Spired<sup>2</sup> Challenge! In this universe, all zones are copies of the Spires! Enemies are a lot stronger in this Challenge<sup>2</sup>!<br/><br/></span>";
			if(game.global.world <= 1)tooltipText += "<br/><hr/><span class='planetBreakDescription'><span class='bad'>If 10 groups of Trimps die in combat while in a spire, the world will return to normal.</span> <span class='good'>You will get a Spire Core Heirloom for every zone cleared in this challenge<sup>2</sup>. It's Spirestone value is boosted depending on the zone number.</span>";
			if(game.global.world > 1)tooltipText += "<span class='planetBreakMessage'>The Real Spire looms menacingly above you, and you take in a deep breath of corruption. You take a look back at your Trimps to help gather some courage, and you push the door open. You slowly walk inside and are greeted by an incredibly loud, deep, human voice.<br/><br/><b>Do you know what you face? If you are defeated ten times in this place, you shall be removed from this space. If you succeed, then you shall see the light of knowledge that you seek.</b><span>";
			if(game.global.world > 1)tooltipText += "<br/><hr/><span class='planetBreakDescription'><span class='bad'>If 10 groups of Trimps die in combat while in a spire, the world will return to normal.</span> <span class='good'>The Spirestone value of the Spire Core Heirloom drop for every Real Spire cleared in this challenge<sup>2</sup> is massive boosted. Every 10th cell cleared in a Real Spire gives a reward, and increases all loot gained until your next portal by 2%.</span>";
			if (game.global.world > 1 && game.options.menu.mapsOnSpire.enabled) tooltipText += "<br/><hr/>You were moved to Maps to protect your limited chances at the spire. You can disable this in settings!";
		}else{
	...
	switch(what) {
		...
		case "Helium":
			...
			if (Fluffy.isRewardActive("radortle")){
				...
				currentCalc *= amt;
				textString += "<tr><td class='bdTitle'>" + Fluffy.getName() + " " + heliumOrRadon() + "</td><td>x 1.03</td><td>" + (Fluffy.isRewardActive("Scruffy21") ? (game.global.highestRadonLevelCleared + 1) : game.global.lastRadonPortal) + "</td><td>+ " + prettify((amt - 1) * 100) + "%</td><td>" + prettify(currentCalc) + "</td></tr>";
			}
